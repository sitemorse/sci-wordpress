	...
	*/
	...
	function establishConnection() {
		if ( $this->proxyHostname ) {
			...
			if ( ! $sock ) {
				throw new Exception(
					'Error connecting to ' . $this->serverHostname .
					':' . $this->serverPort . ' '
				);
			}
		}
	...
	private function sendArgs( $url, $hostNames, $view, $sock, $editurl = '' ) {
		...
		if ( $editurl ) {
			$args['editing'] = $editurl;
		}
		$jsonreq = json_encode( $args );
		$this->fsendall(
			$sock,
			strlen( $jsonreq ) . self::CRLF . $jsonreq .
			self::CRLF
		);
		$line = $this->fgetline( $sock );
		if ( ! $line ) {
	...
	private function fsockopen( $hostname, $port, $errno, $errstr, $timeout ) {
		if ( $this->debug ) {
			$this->debugData .= sprintf(
				"socket open ('%s', %d, %d, %d, %d)\n",
				$hostname,
				$port,
				$errno,
				$errstr,
	...
	private function fsendall( $fp, $s, $log_message = false ) {
		if ( $this->debug ) {
			if ( $log_message ) {
				$this->debugData .= "client: $log_message\n";
			} else {
				$this->debugData .= "client: $s\n";
			}
		}
		...
		while ( true ) {
			...
			*/
			...
			if ( $method == 'POST' && ! $this->postAllowed ) {
				$this->fsendall(
					$sock,
					'XSCI accessdenied POST actions have been' .
			...
			if ( $proto != 'http' && $proto != 'https' ) {
				$this->fsendall(
					$sock,
					"XSCI badscheme URL scheme '" . $proto .
			...
			if ( $i >= count( $this->hostNames ) ) {
				$this->fsendall(
					$sock,
					"XSCI accessdenied CMS proxy access denied to host '"
					. $host . "'" . self::CRLF
				);
				continue;
			}
			...
			|| $port == 19 || $port == 25 ) ) {
				$this->fsendall(
					$sock,
					'XSCI badport Access denied to port ' . $port
					. self::CRLF
				);
				continue;
			}
